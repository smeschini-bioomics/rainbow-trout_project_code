...
# Load limma-corrected proteome matrix and metadata
cat("Loading limma-corrected midgut proteome matrix and metadata...\n")

if (!file.exists(INPUT_RDS)) {
  stop("Corrected proteome RDS not found: ", INPUT_RDS)
}
...
if (!"proteome_batch_corrected_limma" %in% names(rds_obj)) {
  stop(
    "Object 'proteome_batch_corrected_limma' not found in: ",
    INPUT_RDS
  )
}

proteome_df <- as.data.frame(
  rds_obj$proteome_batch_corrected_limma,
...
if (is.null(rownames(proteome_df))) {
  stop("Corrected proteome matrix must have sample IDs as rownames.")
}
...
cat(
  "Corrected proteome matrix:",
  nrow(proteome_df), "samples x",
  ncol(proteome_df), "proteins\n"
)
...
common_ids <- rownames(proteome_df)[
  rownames(proteome_df) %in% meta_df[[SAMPLE_ID_COL]]
]

if (length(common_ids) == 0) {
  stop("No matching sample IDs between proteome matrix and metadata.")
}
...
stopifnot(identical(rownames(proteome_df), rownames(meta_df)))
...
if (
  sum(meta_df[[DIET_COL]] == GROUP_A) < 4 ||
  sum(meta_df[[DIET_COL]] == GROUP_B) < 4
) {
  stop("Need at least 4 samples per diet group.")
}

# Clean proteome matrix
cat("Cleaning midgut proteome matrix...\n")
...
if (any(!keep_nonmissing)) {
  message(
    "Dropping ",
    sum(!keep_nonmissing),
    " protein(s) with <=1 non-missing value globally."
  )
...
if (ncol(proteome_df) < 2) {
  stop("Not enough proteins remain after filtering.")
}

print_matrix_summary(
  proteome_df,
  "Midgut proteome matrix before z-scoring"
)

# Z-score proteins across samples
cat("Z-scoring proteins across samples...\n")

proteome_scaled <- scale(proteome_df)
proteome_scaled <- as.data.frame(proteome_scaled)

rownames(proteome_scaled) <- rownames(proteome_df)
colnames(proteome_scaled) <- colnames(proteome_df)

cat("Sanity check after z-scoring:\n")

col_means <- colMeans(proteome_scaled, na.rm = TRUE)
col_sds <- apply(proteome_scaled, 2, sd, na.rm = TRUE)
...
cat(
  ...
  max(abs(col_means), na.rm = TRUE),
  "\n"
)

cat(
  "Range of SD values:",
  paste(range(col_sds, na.rm = TRUE), collapse = " to "),
  "\n\n"
)

if (APPLY_Z_CAP) {
  proteome_scaled_mat <- as.matrix(proteome_scaled)
  
  proteome_scaled_mat[proteome_scaled_mat > Z_CAP] <- Z_CAP
  proteome_scaled_mat[proteome_scaled_mat < -Z_CAP] <- -Z_CAP
  
  proteome_scaled <- as.data.frame(proteome_scaled_mat)
...
print_matrix_summary(
  proteome_scaled,
  "Z-scored midgut proteome matrix"
)
...
proteome_scaled_pca <- proteome_scaled
proteome_scaled_pca[is.na(proteome_scaled_pca)] <- 0
...
stopifnot(
  identical(
    rownames(proteome_scaled_pca),
    rownames(meta_test)
  )
)

# PERMANOVA
cat("Running PERMANOVA on z-scored midgut proteome profiles...\n")

dist_mat <- dist(proteome_scaled_pca, method = "euclidean")
...
write.csv(
  ...
  file.path(
    DIET_OUTPUT_DIR,
    "midgut_proteome_PERMANOVA_summary.csv"
  ),
  row.names = FALSE
)
...
plot(bd, main = "Midgut proteome betadisper")
dev.off()
...
# PCA directly on z-scored proteome matrix
cat("Running PCA directly on z-scored midgut proteome matrix...\n")
...
pca_df$sample_id <- rownames(proteome_scaled_pca)

pca_df$diet <- meta_test[
  pca_df$sample_id,
  "diet"
]
...
p_pca <- ggplot(
  ...
  labs(
    ...
    title = "Midgut proteome profiles",
...
ggsave(
  file.path(
    DIET_OUTPUT_DIR,
    "midgut_proteome_PCA_with_PC1_PC2_density.pdf"
  ),
  p_pca_density,
  width = 7,
  height = 6
...
ggsave(
  file.path(
    DIET_OUTPUT_DIR,
    "midgut_proteome_PCA_with_PC1_PC2_density.tiff"
  ),
  p_pca_density,
  width = 7,
  height = 6,
...
write.csv(
  ...
  file.path(
    DIET_OUTPUT_DIR,
    "midgut_proteome_PC1_PC2_wilcox_tests.csv"
  ),
  row.names = FALSE
)
...
p_pc_box <- ggplot(
  ...
  labs(
    x = "",
    y = "PC score",
    title = "Diet differences along midgut proteome PCA axes"
  ) +
  theme_cutler() +
  theme(legend.position = "none")
...
ggsave(
  file.path(
    DIET_OUTPUT_DIR,
    "midgut_proteome_PC1_PC2_boxplots.pdf"
  ),
  p_pc_box,
  width = 6,
  height = 4
...
ggsave(
  file.path(
    DIET_OUTPUT_DIR,
    "midgut_proteome_PC1_PC2_boxplots.tiff"
  ),
  p_pc_box,
  width = 6,
  height = 4,
...
write.csv(
  ...
  file.path(
    DIET_OUTPUT_DIR,
    "midgut_proteome_betadisper_summary.csv"
  ),
  row.names = FALSE
)
...
ADDITIONAL_GROUPS <- list(
  ...
  list(
    column = "diet_phase",
    display = "Diet x Temporal Phase",
    folder = "by_diet_phase"
  ),
  list(
    column = "day_categ",
    display = "Day",
...
run_additional_grouping <- function(
    group_column,
    group_display,
    folder_name
) {
  ...
  if (sum(keep_group) < 3) {
    warning(
      "Fewer than 3 samples have non-missing values for ",
      group_column,
  ...
  if (length(group_levels) < 2) {
    warning(
      "Fewer than 2 groups are present for ",
      group_column,
      "; skipping this grouping."
    )
    return(invisible(NULL))
  ...
  pca_group_df$group <- meta_group[
    pca_group_df$sample_id,
    "group"
  ]
  ...
  if (nrow(tested_row_group) < 1) {
    stop(
      "Could not extract the PERMANOVA term for: ",
      group_column,
      ". Returned rows: ",
  ...
  write.csv(
    ...
    file.path(
      group_output_dir,
      "midgut_proteome_PC1_PC2_group_tests.csv"
    ),
    row.names = FALSE
  )
  ...
  p_pc_box_group <- ggplot(
    ...
    geom_text(
      data = label_df_group,
      aes(
        x = (nlevels(meta_group$group) + 1) / 2,
        y = y,
        label = label
      ),
      inherit.aes = FALSE,
    ...
    labs(
      ...
      title = paste0(
        group_display,
        " differences along midgut proteome PCA axes"
      )
    ) +
    theme_cutler() +
    theme(legend.position = "none")
...
names(additional_results) <- vapply(
  ADDITIONAL_GROUPS,
  `[[`,
  character(1),
  "column"
