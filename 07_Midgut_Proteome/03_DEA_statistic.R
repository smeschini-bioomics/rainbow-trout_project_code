...
# Settings
load("01_MSDAP_DEA/2026-04-25_07-16-26/dataset.RData")
...
sample_summary <- dataset$samples %>%
  ...
  mutate(
    diet_clean = diet,
    day = as.integer(str_extract(as.character(day), "\\d+"))
  ) %>%
...
write.csv(
  sample_summary,
  file.path(out_dir, "midgut_sample_n_by_day_diet.csv"),
  row.names = FALSE
)

write.csv(
  sample_total,
  file.path(out_dir, "midgut_sample_n_total.csv"),
  row.names = FALSE
)

write.csv(
  sample_exclusion_summary,
  file.path(out_dir, "midgut_sample_exclusion_summary.csv"),
  row.names = FALSE
)

write.csv(
  reviewer_contrast_table,
  file.path(out_dir, "midgut_reviewer_contrast_table.csv"),
  row.names = FALSE
)

write.csv(
  dea_day_summary,
  file.path(out_dir, "midgut_DEA_day_summary.csv"),
  row.names = FALSE
)

write.csv(
  global_summary,
  file.path(out_dir, "midgut_global_summary.csv"),
  row.names = FALSE
)
