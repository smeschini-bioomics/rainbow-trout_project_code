...
# import data
df <- df <- read.csv("01_input_files/260418_TroutLiver_H3H4_raw_ab_msqrob_input.csv",
                     header=TRUE,
                     skip=0)
ecols = grep("260220", colnames(df))
...
write.csv(
  ...
  file = file.path(OUTDIR, "TroutLiver_peptidoform_H3H4.csv")
)

write.csv(
  ptm_x_assay,
  file = file.path(OUTDIR, "TroutLiver_hPTM_H3H4.csv")
)
