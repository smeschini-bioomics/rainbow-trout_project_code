...
INPUT_MATRIX_RDS <- "02_process_matrix/H3_H4_8PTM_liver.rds"
METADATA_FILE    <- "STPN2309_metadata_all.tsv"

OUTPUT_DIR <- "05_batch_correction_limma"
dir.create(OUTPUT_DIR, recursive = TRUE, showWarnings = FALSE)

SAMPLE_ID_COL <- "sampleID_STPN2309"
DIET_COL      <- "diet"
DAY_COL       <- "day_categ"
BATCH_COL     <- "trout_liver_batch"
...
print_matrix_summary <- function(mat, label = "Matrix") {
  ...
  cat("\n=== ", label, " ===\n", sep = "")
  cat("Dimensions:", nrow(mat), "samples x", ncol(mat), "PTMs\n")
...
if (!file.exists(INPUT_MATRIX_RDS)) {
  stop("PTM matrix RDS not found: ", INPUT_MATRIX_RDS)
}
...
# -----------------------------------------------------------------------------
# Load PTM matrix and metadata
# -----------------------------------------------------------------------------

ptm_df <- readRDS(INPUT_MATRIX_RDS)
ptm_df <- as.data.frame(ptm_df, stringsAsFactors = FALSE)
...
if (is.null(rownames(ptm_df))) {
  stop(
    "The hPTM matrix must have sample IDs stored as rownames."
  )
}
...
if (anyDuplicated(rownames(ptm_df)) > 0) {
  stop("Duplicated sample IDs found in hPTM matrix rownames.")
}
...
# Keep original hPTM matrix sample order
# -----------------------------------------------------------------------------
...
if (length(common_ids) == 0) {
  stop(
    "No common sample IDs found between hPTM matrix rownames and metadata."
  )
}

common_ids <- rownames(ptm_df)[
  rownames(ptm_df) %in% common_ids
]
...
cat("Aligned data:\n")
cat("hPTM matrix:", nrow(ptm_df), "samples x", ncol(ptm_df), "PTMs\n")
...
if (ncol(ptm_df) < 2) {
  stop("Fewer than two PTMs remain after filtering.")
}

print_matrix_summary(
  ptm_df,
  "hPTM matrix before batch correction"
)
...
meta_df[[DAY_COL]] <- factor(meta_df[[DAY_COL]])
...
cat("\nBatch × diet counts:\n")
print(tab_batch_diet)

cat("\nBatch × diet row proportions:\n")
print(round(prop_batch_diet, 3))

cat("\nBatch × day counts:\n")
print(tab_batch_day)

cat("\nBatch × day row proportions:\n")
print(round(prop_batch_day, 3))
...
mat_for_limma <- t(as.matrix(ptm_limma))
...
ptm_corrected <- as.data.frame(t(mat_corrected))

rownames(ptm_corrected) <- rownames(ptm_limma)
colnames(ptm_corrected) <- colnames(ptm_limma)

stopifnot(
  identical(rownames(ptm_limma), rownames(ptm_corrected)),
  identical(colnames(ptm_limma), colnames(ptm_corrected))
)

write.csv(
  cbind(
    sampleID_STPN2309 = rownames(ptm_corrected),
    ptm_corrected
  ),
  file.path(
    OUTPUT_DIR,
    "H3_H4_8PTM_liver_batch_corrected_limma.csv"
  ),
  row.names = FALSE
)
...
#
# No complete-case filtering is applied here, matching your previous script.
# -----------------------------------------------------------------------------
...
pc_assoc_before <- data.frame(
  ...
  p_PC1 = c(
    get_aov_p(
      as.formula(paste("PC1 ~", BATCH_COL)),
      scores_before
    ),
    get_aov_p(
      as.formula(paste("PC1 ~", DIET_COL)),
      scores_before
    ),
    get_aov_p(
      as.formula(paste("PC1 ~", DAY_COL)),
      scores_before
    )
  ),
  p_PC2 = c(
    get_aov_p(
      as.formula(paste("PC2 ~", BATCH_COL)),
      scores_before
    ),
    get_aov_p(
      as.formula(paste("PC2 ~", DIET_COL)),
      scores_before
    ),
    get_aov_p(
      as.formula(paste("PC2 ~", DAY_COL)),
      scores_before
    )
  )
)
...
write.csv(
  ...
  file.path(
    OUTPUT_DIR,
    "PCA_PC_association_tests_before.csv"
  ),
  row.names = FALSE
)
...
write.csv(
  ...
  file.path(
    OUTPUT_DIR,
    "PCA_PC_association_tests_after.csv"
  ),
  row.names = FALSE
)
...
cat(
  "\nBatch-associated PTMs before correction, nominal p < 0.05:",
  sum(batch_check$batch_p_before < 0.05, na.rm = TRUE),
  "\n"
)

cat(
  "Batch-associated PTMs after correction, nominal p < 0.05:",
  sum(batch_check$batch_p_after < 0.05, na.rm = TRUE),
  "\n"
)

cat(
  "Batch-associated PTMs before correction, BH FDR < 0.05:",
  sum(batch_check$batch_FDR_before < 0.05, na.rm = TRUE),
  "\n"
)

cat(
  "Batch-associated PTMs after correction, BH FDR < 0.05:",
...
cat(
  "\nCorrelation of HC - NC PTM differences before versus after correction:",
...
saveRDS(
  list(
    ptm_original = ptm_limma,
    ptm_batch_corrected_limma = ptm_corrected,
    metadata = meta_limma,
...
cat("\nFinal dimensions:\n")
cat("Original matrix:", dim(ptm_limma), "\n")
cat("Corrected matrix:", dim(ptm_corrected), "\n")
