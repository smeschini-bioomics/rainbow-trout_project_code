...
# Output directory
OUTDIR <- "03_batch_effect_pca_exploration"
dir.create(OUTDIR, recursive = TRUE, showWarnings = FALSE)

# INPUTS
INPUT_FILE    <- "02_process_matrix/H3_H4_8PTM_liver.rds"
...
ptm_df <- as.data.frame(readRDS(INPUT_FILE))
...
common_ids <- intersect(rownames(ptm_df), meta_df[[SAMPLE_ID_COL]])

if (length(common_ids) == 0) {
  stop("No shared sample IDs between PTM matrix and metadata.")
}
...
stopifnot(identical(rownames(ptm_df), meta_df[[SAMPLE_ID_COL]]))


# PREPARE MATRIX FOR PCA
ptm_df[] <- lapply(ptm_df, as.numeric)

# Remove PTMs with <= 1 observed value
ptm_df <- ptm_df[
  ,
  colSums(!is.na(ptm_df)) > 1,
  drop = FALSE
]

# Remove constant PTMs
is_constant <- vapply(
  ptm_df,
  function(x) {
    x <- x[!is.na(x)]
...
ptm_df <- ptm_df[, !is_constant, drop = FALSE]

# Median imputation, only for PCA visualization
for (j in seq_len(ncol(ptm_df))) {
  ptm_df[[j]][is.na(ptm_df[[j]])] <- median(
    ptm_df[[j]],
    na.rm = TRUE
  )
}
...
pc1_lab <- paste0("PC1 (", sprintf("%.1f", variance_explained[1]), "%)")
pc2_lab <- paste0("PC2 (", sprintf("%.1f", variance_explained[2]), "%)")
...
ggsave(
  file.path(OUTDIR, "PCA_marginal_density_by_trout_liver_batch.pdf"),
  plot = p_batch,
  width = 8,
  height = 6
)
...
BATCH_REPORT <- file.path(
  OUTDIR,
  "batch_effect_assessment_liver_hPTM.txt"
)

# Standardize PTMs before Euclidean-distance analyses
ptm_for_test <- scale(ptm_df, center = TRUE, scale = TRUE)
dist_ptm <- dist(ptm_for_test, method = "euclidean")
...
disp_batch <- vegan::betadisper(
  dist_ptm,
  group = meta_df$trout_liver_batch
)
...
permanova_adjusted <- vegan::adonis2(
  dist_ptm ~ diet + day_categ + trout_liver_batch,
  data = meta_df,
  permutations = 9999,
  by = "margin"
)
...
# ---------------------------------------------------------
# 4. Batch × day allocation
# ---------------------------------------------------------
tab_batch_day <- table(
  meta_df$trout_liver_batch,
  meta_df$day_categ
)
...
design_check <- model.matrix(
  ~ trout_liver_batch + diet * day_categ,
  data = meta_df
)
...
report_lines <- capture.output({
  ...
  cat("liver hPTM BATCH-EFFECT ASSESSMENT\n")
  cat("==================================\n\n")
  
  cat("Input matrix:\n")
  cat("- Samples:", nrow(ptm_df), "\n")
  cat("- PTMs:", ncol(ptm_df), "\n")
  cat("- Distance: Euclidean on feature-wise standardized PTM values\n")
  ...
  if (design_rank["model_rank"] == design_rank["n_model_columns"]) {
    cat(
      ...
      "The model trout_liver_batch + diet * day_categ is estimable ",
  ...
  cat("  ~ trout_liver_batch + diet * day_categ\n")
})
