...
histone_metadata <- histone_metadata %>%
  mutate(
    ...
    liver_hptm_batch = factor(.data[[BATCH_FIXED_COL]])
  )

histone_metadata$diet <- droplevels(histone_metadata$diet)
histone_metadata$day_categ <- droplevels(histone_metadata$day_categ)
histone_metadata$liver_hptm_batch <- droplevels(
  histone_metadata$liver_hptm_batch
)
...
if (nlevels(histone_metadata$liver_hptm_batch) < 2) {
  stop("liver_hptm_batch has fewer than two levels.")
}
...
cat("\nBatch counts:\n")
print(table(histone_metadata$liver_hptm_batch))
...
fit_brms_one <- function(feature,
  ...
  dat_feat <- dat_feat %>%
    mutate(
      ...
      liver_hptm_batch = factor(liver_hptm_batch)
    )
  
  if (nlevels(dat_feat$liver_hptm_batch) < 2) {
    warning("Feature ", feature, ": liver_hptm_batch has fewer than 2 levels after NA filtering – skipping.")
    return(NULL)
  }
  ...
  fit <- brm(
    formula    = bf(response ~ day_categ * diet + liver_hptm_batch),
    data       = dat_feat,
    family     = fam_obj,
    prior      = priors,
    iter       = iter,
  ...
  diag_row <- tibble(
    feature      = feature,
    family       = fam_name,
    formula_used = "response ~ day_categ * diet + liver_hptm_batch",
...
# WITH liver_hptm_batch as fixed effect
...
fit_best_final_one <- function(feature,
  ...
  writeLines(
    c(
      ...
      "formula: response ~ day_categ * diet + liver_hptm_batch"
    ),
