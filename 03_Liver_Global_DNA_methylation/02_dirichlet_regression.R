...
# Save sample-size and descriptive statistics
sample_size_dir <- file.path("results", "sample_size")
...
# Create the Dirichlet regression output folder
out_dir <- file.path("results", "dirichlet_regression")
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

# Calculate prior intercept centres from NC at day 1
df_base <- df %>%
  filter(diet == "NC", day == "day_01")

mu_md_vs_dC <- log(mean(df_base$mdC_rel) / mean(df_base$dC_rel))
mu_hmd_vs_dC <- log(mean(df_base$hmdC_rel) / mean(df_base$dC_rel))

cat(
  "NC day_01 intercepts:\n",
  "mu_md_vs_dC =", round(mu_md_vs_dC, 6), "\n",
  "mu_hmd_vs_dC =", round(mu_hmd_vs_dC, 6), "\n"
)
...
# Set intercept priors
prior_tbl$prior[
  prior_tbl$class == "Intercept" &
    prior_tbl$dpar == "mumdCrel"
] <- sprintf("student_t(3, %.3f, 1)", mu_md_vs_dC)

prior_tbl$prior[
  prior_tbl$class == "Intercept" &
    prior_tbl$dpar == "muhmdCrel"
] <- sprintf("student_t(3, %.3f, 1)", mu_hmd_vs_dC)
