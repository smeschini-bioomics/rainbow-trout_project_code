...
- `input_files/` — HPLC–UV global cytosine abundance measurements and the derived composition table.
- `results/dirichlet_regression/` — fitted Bayesian model, posterior summaries, HC-minus-NC contrasts, heatmap, diagnostic outputs, and session information.
- `results/beeswarm_plot/` — raw-data beeswarm figures, sample-count tables, and descriptive summaries.
- `results/CV_analysis/` — coefficient-of-variation table and figure.
- `results/sample_size/` — composition summary by diet and day, including sample counts.

Run scripts from `03_Liver_Global_DNA_methylation/`.

## Inputs and composition preparation

The raw input file is:

```text
input_files/methylome.tsv
```

It must contain:

- `dC` — unmodified cytosine measurement;
- `mdC` — 5-methylcytosine measurement;
- `hmdC` — 5-hydroxymethylcytosine measurement;
- `diet`;
- `day`.

`01_preparing_data.R` calculates the total measured cytosine signal per sample:

```r
total_cytosine = dC + mdC + hmdC
```

and derives the three relative fractions:
...
The three fractions represent a closed composition. For every sample included in the Dirichlet model, they must be finite and strictly between zero and one.

The comparison is HC versus NC over days 1, 2, 3, 4, 10, 15, and 22. The model script expects the corresponding day labels, including `day_01` for the baseline prior calibration.
