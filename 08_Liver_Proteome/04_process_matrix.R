...
proteome <- fread(
  "01_MSDAP_DEA/2026-04-29_16-53-21/protein_abundance__global data filter.tsv"
)
...
stopifnot("sampleID_progentomics_proteome_liver" %in% colnames(metadata))

sample_id_map <- setNames(
  rownames(metadata),
  metadata$sampleID_progentomics_proteome_liver
)
...
if (length(unmatched_sample_ids) > 0) {
  stop(
    "Proteome sample IDs not found in metadata$sampleID_progentomics_proteome_liver: ",
...
saveRDS(
  proteome_filtered,
  file = file.path(RDS_DIR, "liver_proteome_filtered.rds")
)
