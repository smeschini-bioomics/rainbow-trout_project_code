...
cat("Loading limma-corrected liver proteome matrix and metadata...\n")
...
cat("Cleaning liver proteome matrix...\n")
...
print_matrix_summary(
  proteome_df,
  "liver proteome matrix before z-scoring"
)
...
print_matrix_summary(
  proteome_scaled,
  "Z-scored liver proteome matrix"
)
...
# PERMANOVA
cat("Running PERMANOVA on z-scored liver proteome profiles...\n")
...
write.csv(
  ...
  file.path(
    DIET_OUTPUT_DIR,
    "liver_proteome_PERMANOVA_summary.csv"
  ),
  row.names = FALSE
)
...
plot(bd, main = "liver proteome betadisper")
dev.off()
...
cat("Running PCA directly on z-scored liver proteome matrix...\n")
...
ggsave(
  file.path(
    DIET_OUTPUT_DIR,
    "liver_proteome_PCA_with_PC1_PC2_density.pdf"
  ),
  p_pca_density,
  width = 7,
  height = 6
...
ggsave(
  file.path(
    DIET_OUTPUT_DIR,
    "liver_proteome_PCA_with_PC1_PC2_density.tiff"
  ),
  p_pca_density,
  width = 7,
  height = 6,
...
write.csv(
  ...
  file.path(
    DIET_OUTPUT_DIR,
    "liver_proteome_PC1_PC2_wilcox_tests.csv"
  ),
  row.names = FALSE
)
...
p_pc_box <- ggplot(
  ...
  labs(
    x = "",
    y = "PC score",
    title = "Diet differences along liver proteome PCA axes"
  ) +
  theme_cutler() +
  theme(legend.position = "none")
...
ggsave(
  file.path(
    DIET_OUTPUT_DIR,
    "liver_proteome_PC1_PC2_boxplots.pdf"
  ),
  p_pc_box,
  width = 6,
  height = 4
...
ggsave(
  file.path(
    DIET_OUTPUT_DIR,
    "liver_proteome_PC1_PC2_boxplots.tiff"
  ),
  p_pc_box,
  width = 6,
  height = 4,
...
write.csv(
  ...
  file.path(
    DIET_OUTPUT_DIR,
    "liver_proteome_betadisper_summary.csv"
  ),
  row.names = FALSE
)
...
run_additional_grouping <- function(
  ...
  write.csv(
    ...
    file.path(
      group_output_dir,
      "liver_proteome_PC1_PC2_group_tests.csv"
    ),
    row.names = FALSE
  )
  ...
  p_pc_box_group <- ggplot(
    ...
    labs(
      ...
      title = paste0(
        group_display,
        " differences along liver proteome PCA axes"
      )
    ) +
    theme_cutler() +
    theme(legend.position = "none")
