...
# INPUTS
INPUT_FILE    <- file.path("rds", "liver_proteome_filtered.rds")
...
p_batch <- make_pca_density_plot(
  scores_df,
  group_var = BATCH_COL,
  group_label = "liver proteome batch"
)
...
BATCH_REPORT <- file.path(
  OUTDIR,
  "batch_effect_assessment_liver_proteome.txt"
)
...
permanova_batch <- vegan::adonis2(
  dist_proteome ~ trout_liver_batch,
  data = meta_df,
  permutations = 9999
)
...
permanova_adjusted <- vegan::adonis2(
  dist_proteome ~ diet + day_categ + trout_liver_batch,
  data = meta_df,
  permutations = 9999,
  by = "margin"
)
...
report_lines <- capture.output({
  ...
  cat("liver PROTEOME BATCH-EFFECT ASSESSMENT\n")
  cat("========================================\n\n")
