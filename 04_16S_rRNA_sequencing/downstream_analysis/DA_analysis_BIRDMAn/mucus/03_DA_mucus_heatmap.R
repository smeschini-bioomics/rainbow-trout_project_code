...
pd_keep_thresh  <- 0.975
pd_label_thresh <- 0.975
...
feature_labels <- heat_df2 %>%
  distinct(feature, tax_label_plot) %>%
  tibble::deframe()
...
p_dot <- ggplot(heat_df2, aes(x = day_label, y = feature)) +
  ...
  labs(
    #title = "Mucus-Associated Microbiota DA Analysis",
    x = NULL, y = NULL
  ) +
...
ggsave(
  file.path(PLOT_OUTDIR, "dotplot_diet_effect_mucus_microbiome.tiff"),
  plot_grid_out_title,
  device = "tiff",
  width = 12,
  height = 9,
...
ggsave(
  file.path(PLOT_OUTDIR, "dotplot_diet_effect_mucus_microbiome.pdf"),
  plot_grid_out_title,
  device = "pdf",
  width = 12,
  height = 9,
