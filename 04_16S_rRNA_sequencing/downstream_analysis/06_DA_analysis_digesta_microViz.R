...
out_dir <- "06_DA_analysis_digesta_microViz"
if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE)

# Import filtered BIOM table used for DA analysis in BIRDMAn
ps <- suppressWarnings(
  import_biom("DA_analysis_BIRDMAn/digesta/output_files/digesta_filtered_table_with_sample_metadata.biom")
)
...
tree_labelled_sig <- bb_stats %>%
  ...
  taxatree_plots(
    ...
    var_renamer = function(x) "Digesta-Associated Microbiota HC vs NC"
  ) %>%
  .[[1]] %>%
