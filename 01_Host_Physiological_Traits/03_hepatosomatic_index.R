...
# Load file
file_path <- "input_files/hepatosomatic_index.tsv"
...
ggplot(data_no_fasted, aes(x = HSI)) +
  ...
  geom_vline(aes(xintercept = mean(HSI, na.rm = TRUE)), 
             color = "darkgreen", linetype = "dashed", linewidth = 1) +
  
  # Vertical line for the median
  geom_vline(aes(xintercept = median(HSI, na.rm = TRUE)), 
             color = "orange", linetype = "dotted", linewidth = 1) +
  
  # Labels and theme
  labs(
    title = "Distribution of Hepato-Somatic Index (HSI) (%)",
    subtitle = "Histogram with density curve, mean (dashed), and median (dotted)",
    x = "Hepato-Somatic Index (HSI) (%)",
    y = "Density"
  ) +
...
stats_by_diet <- data_no_fasted %>%
  group_by(diet) %>%
  summarise(
    mean_lla   = mean(HSI, na.rm = TRUE),
    median_lla = median(HSI, na.rm = TRUE),
    logmean_lla = exp(mean(log(HSI[HSI > 0]), na.rm = TRUE)),
    .groups = "drop"
  )
...
yrng <- range(data_plot$HSI, na.rm = TRUE)
pad  <- diff(yrng) * 0.08

sig_df <- data_plot %>%
  group_by(day) %>%
  summarise(
    y_position = max(HSI, na.rm = TRUE) + pad,
    .groups = "drop"
  ) %>%
  left_join(
    all_contrasts %>%
...
p_HSI <- ggplot(data_plot, aes(x = x_pos, y = HSI)) +
