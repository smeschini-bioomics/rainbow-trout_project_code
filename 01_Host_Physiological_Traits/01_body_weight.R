...
# Load file
file_path <- "input_files/body_weight.tsv"
...
ggplot(data_no_fasted, aes(x = body_weight)) +
  # Histogram of HSI (scaled to density)
  ...
  geom_vline(aes(xintercept = mean(body_weight, na.rm = TRUE)), 
             color = "darkgreen", linetype = "dashed", linewidth = 1) +
  
  # Vertical line for the median
  geom_vline(aes(xintercept = median(body_weight, na.rm = TRUE)), 
             color = "orange", linetype = "dotted", linewidth = 1) +
  
  # Labels and theme
  labs(
    title = "Distribution of Body Weight (g)",
...
stats_by_diet <- data_no_fasted %>%
  group_by(diet) %>%
  summarise(
    mean_lla   = mean(body_weight, na.rm = TRUE),
    median_lla = median(body_weight, na.rm = TRUE),
    logmean_lla = exp(mean(log(body_weight[body_weight > 0]), na.rm = TRUE)),
    .groups = "drop"
  )
...
if (any(data_no_fasted$body_weight <= 0, na.rm = TRUE)) {
  stop("body_weight contains zero or negative values. Log-Gaussian and Gamma models require positive values.")
}


model_gaussian <- glmmTMB(
  body_weight ~ day * diet,
  data = data_no_fasted,
...
# Compare models within the same family
...
# Pairwise contrasts for Body Weight
...
yrng <- range(data_plot$body_weight, na.rm = TRUE)
pad  <- diff(yrng) * 0.08

sig_df <- data_plot %>%
  group_by(day) %>%
  summarise(
    y_position = max(body_weight, na.rm = TRUE) + pad,
    .groups = "drop"
  ) %>%
  left_join(
    all_contrasts %>%
...
p_BW <- ggplot(data_plot, aes(x = x_pos, y = body_weight)) +
