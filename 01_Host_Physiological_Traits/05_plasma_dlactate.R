...
# Load file
file_path <- "input_files/plasma_dlactate.tsv"
...
# transform mM in uM
data_no_fasted$Plasma_D_Lactate <- data_no_fasted$Plasma_D_Lactate * 1000

# Data Exploratory Visualization
ggplot(data_no_fasted, aes(x = Plasma_D_Lactate)) +
  # Histogram of Plasma D_Lactate (scaled to density)
  ...
  geom_vline(aes(xintercept = mean(Plasma_D_Lactate, na.rm = TRUE)), 
             color = "darkgreen", linetype = "dashed", linewidth = 1) +
  
  # Vertical line for the median
  geom_vline(aes(xintercept = median(Plasma_D_Lactate, na.rm = TRUE)), 
             color = "orange", linetype = "dotted", linewidth = 1) +
  
  # Labels and theme
  labs(
    title = "Distribution of Plasma D_Lactate (mM)",
    subtitle = "Histogram with density curve, mean (dashed), and median (dotted)",
    x = "Plasma D_Lactate (mM)",
    y = "Density"
  ) +
...
stats_by_diet <- data_no_fasted %>%
  group_by(diet) %>%
  summarise(
    mean_lla   = mean(Plasma_D_Lactate, na.rm = TRUE),
    median_lla = median(Plasma_D_Lactate, na.rm = TRUE),
    logmean_lla = exp(mean(log(Plasma_D_Lactate[Plasma_D_Lactate > 0]), na.rm = TRUE)),
    .groups = "drop"
  )
...
# Check positivity for log-Gaussian and Gamma models
if (any(data_no_fasted$Plasma_D_Lactate <= 0, na.rm = TRUE)) {
  stop("Plasma_D_Lactate contains zero or negative values. Log-Gaussian and Gamma models require positive values.")
}


model_gaussian <- glmmTMB(
  Plasma_D_Lactate ~ day * diet,
  data = data_no_fasted,
...
# Compare models within the same family / same response scale
...
yrng <- range(data_plot$Plasma_D_Lactate, na.rm = TRUE)
pad  <- diff(yrng) * 0.08

sig_df <- data_plot %>%
  group_by(day) %>%
  summarise(
    y_position = max(Plasma_D_Lactate, na.rm = TRUE) + pad,
    .groups = "drop"
  ) %>%
  left_join(
    all_contrasts %>%
...
p_D_La <- ggplot(data_plot, aes(x = x_pos, y = Plasma_D_Lactate)) +
