...
# Load file
file_path <- "input_files/plasma_glucose.tsv"
...
# Pivoting from wide to long format because repeated measures are the values
data_long <- data %>%
  pivot_longer(cols = starts_with("rep"), # Select replicate columns
               names_to = "replicate",    # New column for replicate ID
               values_to = "glucose")     # New column for glucose values
...
# Add a new column with the median of the replicates
data <- data %>%
  mutate(glucose_median = apply(select(., replicate_1, replicate_2, replicate_3), 1, median, na.rm = TRUE))
# Add a new column with the mean of the replicates
data <- data %>%
  mutate(plasma_glucose = rowMeans(select(., replicate_1, replicate_2, replicate_3), na.rm = TRUE))
...
ggplot(data_no_fasted, aes(x = plasma_glucose)) +
  # Histogram of plasma_glucose (scaled to density)
  ...
  geom_vline(aes(xintercept = mean(plasma_glucose, na.rm = TRUE)), 
             color = "darkgreen", linetype = "dashed", linewidth = 1) +
  
  # Vertical line for the median
  geom_vline(aes(xintercept = median(plasma_glucose, na.rm = TRUE)), 
             color = "orange", linetype = "dotted", linewidth = 1) +
  
  # Labels and theme
  labs(
    title = "Distribution of Plasma Glucose concentration (g/L)",
...
stats_by_diet <- data_no_fasted %>%
  group_by(diet) %>%
  summarise(
    mean_lla   = mean(plasma_glucose, na.rm = TRUE),
    median_lla = median(plasma_glucose, na.rm = TRUE),
    logmean_lla = exp(mean(log(plasma_glucose[plasma_glucose > 0]), na.rm = TRUE)),
    .groups = "drop"
  )
...
# Compare only models from the same family / same response scale
...
day_labs <- setNames(
  paste("Day", gsub("^Day\\s*", "", levels(data_no_fasted$day))),
  levels(data_no_fasted$day)
)
...
yrng <- range(data_plot$plasma_glucose, na.rm = TRUE)
pad  <- diff(yrng) * 0.08

sig_df <- data_plot %>%
  group_by(day) %>%
  summarise(
    y_position = max(plasma_glucose, na.rm = TRUE) + pad,
    .groups = "drop"
  ) %>%
  left_join(
    all_contrasts %>%
...
p_glucose <- ggplot(data_plot, aes(x = x_pos, y = plasma_glucose)) +
