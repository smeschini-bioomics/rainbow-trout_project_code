...
# Wide table: one row per gene, day, with HC and NC sample sizes
n_summary_gene_day <- sample_size_table %>%
  mutate(
    day_num = as.numeric(gsub("[^0-9]", "", day)),
    day = paste0("Day ", day_num)
  ) %>%
  select(gene, day_num, day, diet, n) %>%
  tidyr::pivot_wider(
    names_from = diet,
    values_from = n,
    values_fill = 0
  ) %>%
  arrange(gene, day_num)
# publication legend sample size
txt <- n_summary_gene_day %>%
  mutate(
    txt = paste0(day, " = HC ", HC, "/NC ", NC)
  ) %>%
  group_by(gene) %>%
  summarise(
...
publication_txt <- paste0(
  "Sample sizes are reported as HC/NC for each time point. ",
  paste(txt, collapse = "; "),
  "."
)
...
writeLines(
  publication_txt,
  file.path(out_dir, "sample_sizes_publication_text.txt")
)
