...
# First metadata column contains sample IDs
...
fit_student_one <- function(
  ...
  dat_feat$gut_hptm_batch <- droplevels(
    dat_feat$gut_hptm_batch
  )
  ...
  # Same fixed-effect structure as the Bayesian model
  fit <- glmmTMB(
  ...
  # Estimated marginal means by diet within each day
  # component = "cond" is not required here because it is the default.
  emm <- emmeans(
    fit,
    ~ diet | day_categ
  )
  ...
  diagnostics <- tibble(
    ...
    positive_definite_hessian = isTRUE(
      fit$sdr$pdHess
    ),
    logLik = as.numeric(
      logLik(fit)
...
# Fit every PTM using the Student-t family
results <- vector(
  mode = "list",
...
if (length(successful_results) == 0) {
  stop(
    "No Student-t models were fitted successfully. ",
...
# Frequentist Student-t output directories
base_dir <- file.path(
  "05_glmmTMB_student_t_regression",
  "midgut_hPTM_batch_fixed"
)
...
# Prepare contrasts
# BH adjustment is performed separately for each PTM
# across its seven HC - NC day-specific contrasts.
contrast_df <- contrast_df %>%
  mutate(
...
# Save the exact FDR-adjusted table used for plotting
readr::write_csv(
  contrast_df,
  file.path(
    summary_dir,
    "all_features_student_t_HC_minus_NC_by_day_heatmap_FDR.csv"
  )
)
...
saveRDS(
  ...
  file.path(
    summary_dir,
    "hptm_midgut_student_t_median_abs_delta.rds"
  )
)
...
readr::write_csv(
  ...
  file.path(
    summary_dir,
    "hptm_midgut_student_t_median_abs_delta.csv"
  )
)
...
p_activation <- ggplot(
  ...
  labs(
    ...
    title =
      "Frequentist Student-t: Midgut hPTM effect dynamics"
  ) +
  theme_grey(
    base_size = 13
  ) +
...
# Cluster PTMs using the HC - NC estimates across days
heat_df <- contrast_df %>%
  mutate(
    day_num = as.numeric(
      gsub(
...
dendrogram_object <- as.dendrogram(hc)
...
# Size: absolute HC - NC estimate
# Text: FDR significance symbols
#
# The confidence-interval outline used in the Bayesian figure
# is intentionally omitted from this frequentist sensitivity plot.
p_dot <- ggplot(
  heat_df2,
  aes(
    x = day_label,
