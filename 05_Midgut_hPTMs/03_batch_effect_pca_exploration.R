...
# INPUTS
INPUT_FILE    <- "02_process_matrix/H3_H4_5PTM_midgut.rds"
...
ggsave(
  file.path(OUTDIR, "PCA_marginal_density_by_gut_hptm_batch.pdf"),
  plot = p_batch,
  width = 8,
  height = 6
)
...
BATCH_REPORT <- file.path(
  OUTDIR,
  "batch_effect_assessment_midgut_hPTM.txt"
)
...
permanova_adjusted <- vegan::adonis2(
  dist_ptm ~ diet + day_categ + gut_hptm_batch,
  data = meta_df,
  permutations = 9999,
  by = "margin"
)
...
report_lines <- capture.output({
  ...
  cat("MIDGUT hPTM BATCH-EFFECT ASSESSMENT\n")
  cat("==================================\n\n")
  ...
  if (design_rank["model_rank"] == design_rank["n_model_columns"]) {
    cat(
      ...
      "The model gut_hptm_batch + diet * day_categ is estimable ",
  ...
  cat("  ~ gut_hptm_batch + diet * day_categ\n")
})
